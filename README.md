...
Более абстрактное описание проекта, и при этом более теоретически углубленное в течение двух месяцев(я надеюсь) появится на habr`е. Ссылка на статю -> [тут будет ссылка на статью]. Данная реализация не является потокобезопасной, и фактически возможность создавать потоки добавлена исключительно ради многопоточности, но при этом без асинхронности. Возможно в будущем доработаю и добавлю потокобезопасноть - это было бы следующим крепким шагом на пути к осознанию. 
